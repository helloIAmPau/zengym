...
if [ ! -f /data/postgresql.conf ]; then
  chown postgres /data
  gosu postgres bash -c '/usr/lib/postgresql/17/bin/initdb -D /data --username "$POSTGRES_USER" --pwfile=<(printf "%s\n" "$POSTGRES_PASSWORD")'
...
gosu postgres bash -c ". /python-env/bin/activate && /usr/lib/postgresql/17/bin/postgres -h 0.0.0.0 -D /data"
